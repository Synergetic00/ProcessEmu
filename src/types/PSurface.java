...
/*
  Part of the Processing project - http://org
...
public class PSurface {
		...
		KeyFrame keyFrame = new KeyFrame(Duration.millis(1000),
			...
			public void handle(ActionEvent event) {
				...
				if (sketch.frameCount > 5) {
					animation.setRate(-min(1e9f / drawNanos, frameRate));
				}
			}
		});
	...
	public void initOffscreen(FXApp sketch) {
	}


	//  public Component initComponent(FXApp sketch) {
	//    return null;
	//  }


	static public class PApplicationFX extends Application {
		static public PSurface surface;
		...
		public void start(final Stage stage) {
			...
			boolean spanDisplays = sketch.sketchDisplay() == SPAN;
			...
			// Set the displayWidth/Height variables inside FXApp, so that they're
			...
			if ((FXApp.platform == MACOSX ||
					FXApp.platform == LINUX) &&
					FXApp.javaVersionName.compareTo("1.8.0_60") >= 0 &&
					FXApp.javaVersionName.compareTo("1.8.0_72") < 0) {
	...
	//public Frame initFrame(FXApp sketch, java.awt.Color backgroundColor,
	public void initFrame(FXApp sketch) {/*, int backgroundColor,
	...
	protected void setProcessingIcon(Stage stage) {
		...
		try {
			if (iconImages == null) {
				...
				for (int sz : sizes) {
					URL url = FXApp.class.getResource("/icon/icon-" + sz + ".png");
...
//    setupFrameResizeListener();
  ...
  protected void addListeners() {
	...
	static {
		mouseMap.put(MouseEvent.MOUSE_PRESSED, event.MouseEvent.PRESS);
		mouseMap.put(MouseEvent.MOUSE_RELEASED, event.MouseEvent.RELEASE);
		mouseMap.put(MouseEvent.MOUSE_CLICKED, event.MouseEvent.CLICK);
		mouseMap.put(MouseEvent.MOUSE_DRAGGED, event.MouseEvent.DRAG);
		mouseMap.put(MouseEvent.MOUSE_MOVED, event.MouseEvent.MOVE);
		mouseMap.put(MouseEvent.MOUSE_ENTERED, event.MouseEvent.ENTER);
		mouseMap.put(MouseEvent.MOUSE_EXITED, event.MouseEvent.EXIT);
	}
	...
	protected void fxMouseEvent(MouseEvent fxEvent) {
		...
		if (fxEvent.isShiftDown()) {
			modifiers |= event.Event.SHIFT;
		}
		if (fxEvent.isControlDown()) {
			modifiers |= event.Event.CTRL;
		}
		if (fxEvent.isMetaDown()) {
			modifiers |= event.Event.META;
		}
		if (fxEvent.isAltDown()) {
			modifiers |= event.Event.ALT;
		}
		...
		sketch.postEvent(new event.MouseEvent(fxEvent, when,
				action, modifiers,
				x, y, button, count));
	}
	...
	protected void fxScrollEvent(ScrollEvent fxEvent) {
		...
		int action = event.MouseEvent.WHEEL;
	...
	protected void fxKeyEvent(javafx.scene.input.KeyEvent fxEvent) {
		...
		if (et == KeyEvent.KEY_PRESSED) {
			action = event.KeyEvent.PRESS;
		} else if (et == KeyEvent.KEY_RELEASED) {
			action = event.KeyEvent.RELEASE;
		} else if (et == KeyEvent.KEY_TYPED) {
			action = event.KeyEvent.TYPE;
		}
		...
		sketch.postEvent(new event.KeyEvent(fxEvent, when,
				action, modifiers,
				keyChar, keyCode));
	}
