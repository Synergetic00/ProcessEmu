...
void drawNode(Node ni, float x, float y, int toImage) {
  ...
  if (toImage == 0) {
    ...
    textAlign(CENTER);
    textSize(0.4*ni.m*scaleToFixBug);
    ...
    screenImage.textSize(0.4*ni.m*scaleToFixBug);
    ...
    popUpImage.textSize(0.4*ni.m*scaleToFixBug);
...
void drawMuscleAxons(Muscle mi, ArrayList<Node> n, float x, float y, int toImage) {
  ...
  if(mi.axon >= 0 && mi.axon < n.size()){
    ...
    if(toImage == 0){
      fill(axonColor);
      textAlign(CENTER);
      textSize(0.4*averageMass*scaleToFixBug);
      ...
      screenImage.textAlign(CENTER);
      screenImage.textSize(0.4*averageMass*scaleToFixBug);
      ...
      popUpImage.textAlign(CENTER);
      popUpImage.textSize(0.4*averageMass*scaleToFixBug);
...
void drawPosts(int toImage) {
  ...
  if (toImage == 0) {
    noStroke();
    textAlign(CENTER);
    textSize(postFontSize*scaleToFixBug); 
    ...
    popUpImage.textAlign(CENTER);
    popUpImage.textSize(postFontSize*scaleToFixBug); 
    popUpImage.noStroke();
    for(int postY = startPostY; postY <= startPostY+8; postY += 4){
      for (int i = (int)(averageX/5-5); i <= (int)(averageX/5+5); i++) {
        popUpImage.fill(255);
