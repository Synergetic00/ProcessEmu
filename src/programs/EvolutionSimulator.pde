final double windowSizeMultiplier = 0.8;
final int SEED = 0;
...
double bigMutationChance = 0.06;
double hazelStairs = -1;
boolean saveFramesPerGeneration = true;

int lastImageSaved = -1;
double pressureUnit = 500.0/2.37;
double energyUnit = 20;
double nauseaUnit = 5;
int minBar = -10;
int maxBar = 100;
...
int gensToDo = 0;
double cTimer = 60;
double postFontSize = 0.96;
double scaleToFixBug = 1000;
double energy = 0;
double averageNodeNausea = 0;
double totalNodeNausea = 0;

double lineY1 = -0.08; // These are for the lines of text on each node.
double lineY2 = 0.35;
...
double camZoom = 0.015;
double gravity = 0.005;
double airFriction = 0.95;
...
double inter(int a, int b, double offset) {
  return (double)(a)+((double)(b)-(double)(a))*offset;
}
double r() {
...
class Rectangle {
  double x1, y1, x2, y2;
  Rectangle(double tx1, double ty1, double tx2, double ty2) {
    x1 = tx1;
    y1 = ty1;
    x2 = tx2;
    y2 = ty2;
...
class Node {
  double x, y, vx, vy, prevX, prevY, pvx, pvy, m, f, value, valueToBe;
  int operation, axon1, axon2;
  boolean safeInput;
  double pressure;
  Node(double tx, double ty, double tvx, double tvy, double tm, double tf, double val, int op, int a1, int a2) {
    prevX = x = tx;
    prevY = y = ty;
    pvx = vx = tvx;
    pvy = vy = tvy;
  ...
  void pressAgainstGround(double groundY){
    double dif = y-(groundY-m/2);
  ...
  void hitWalls() {
    ...
    for (int i = 0; i < rects.size(); i++) {
      ...
      if (abs(x-(r.x1+r.x2)/2) <= (r.x2-r.x1+m)/2 && abs(y-(r.y1+r.y2)/2) <= (r.y2-r.y1+m)/2) {
        ...
        double distance = dist(x, y, px, py);
        double rad = m/2;
        double wallAngle = atan2(py-y, px-x);
        if (flip) {
          wallAngle += PI;
        }
        if (distance < rad || flip) {
          dif = rad-distance;
          pressure += dif*pressureUnit;
          double multi = rad/distance;
          if (flip) {
            multi = -multi;
          }
          x = (x-px)*multi+px;
          y = (y-py)*multi+py;
          double veloAngle = atan2(vy, vx);
          double veloMag = dist(0, 0, vx, vy);
          double relAngle = veloAngle-wallAngle;
          double relY = sin(relAngle)*veloMag*dif*FRICTION;
  ...
  void doMath(int i, ArrayList<Node> n){
    double axonValue1 = n.get(axon1).value;
    double axonValue2 = n.get(axon2).value;
  ...
  Node modifyNode(double mutability, int nodeNum) {
    double newX = x+r()*0.5*mutability;
    double newY = y+r()*0.5*mutability;
    double newM = m+r()*0.1*mutability;
    newM = min(max(newM, 0.3), 0.5);
    newM = 0.4;
    
    double newV = value*(1+r()*0.2*mutability);
...
class Muscle {
  ...
  Muscle(int taxon, int tc1, int tc2, double tlen, double trigidity) {
    axon  = taxon;
  ...
  void applyForce(int i, ArrayList<Node> n) {
    ...
    Node ni1 = n.get(c1);
    Node ni2 = n.get(c2);
    double distance = dist(ni1.x, ni1.y, ni2.x, ni2.y);
    double angle = atan2(ni1.y-ni2.y, ni1.x-ni2.x);
  ...
  Muscle modifyMuscle(int nodeNum, double mutability) {
    int newc1 = c1;
    int newc2 = c2;
    int newAxon = axon;
    if(random(0,1)<bigMutationChance*mutability){
    ...
    double newR = min(max(rigidity*(1+r()*0.9*mutability),0.01),0.08);
    double newLen = min(max(len+r()*mutability,0.4),1.25);
...
class Creature {
  ...
  int id;
  boolean alive;
  double creatureTimer;
  double mutability;
  
  Creature(int tid, ArrayList<Node> tn, ArrayList<Muscle> tm, double td, boolean talive, double tct, double tmut) {
    id = tid;
    m = tm;
    n = tn;
    d = td;
  ...
  void addRandomNode() {
    ...
    double ang1 = random(0, 2*PI);
    double distance = sqrt(random(0, 1));
    double x = n.get(parentNode).x+cos(ang1)*0.5*distance;
    double y = n.get(parentNode).y+sin(ang1)*0.5*distance;
    ...
    for (int i = 0; i < n.size()-1; i++) {
      if (i != parentNode) {
        double dx = n.get(i).x-x;
        double dy = n.get(i).y-y;
...
void drawNode(Node ni, double x, double y, int toImage) {
...
void drawNodeAxons(ArrayList<Node> n, int i, double x, double y, int toImage) {
  Node ni = n.get(i);
  if(operationAxons[ni.operation] >= 1){
    Node axonSource = n.get(n.get(i).axon1);
    double point1x = ni.x-ni.m*0.3+x;
    double point1y = ni.y-ni.m*0.3+y;
    double point2x = axonSource.x+x;
    double point2y = axonSource.y+axonSource.m*0.5+y;
...
void drawSingleAxon(double x1, double y1, double x2, double y2, int toImage){
  double arrowHeadSize = 0.1;
...
void drawMuscle(Muscle mi, ArrayList<Node> n, double x, double y, int toImage) {
...
void drawMuscleAxons(Muscle mi, ArrayList<Node> n, double x, double y, int toImage) {
  Node ni1 = n.get(mi.c1);
  Node ni2 = n.get(mi.c2);
  if(mi.axon >= 0 && mi.axon < n.size()){
    Node axonSource = n.get(mi.axon);
    double muscleMidX = (ni1.x+ni2.x)*0.5+x;
    double muscleMidY = (ni1.y+ni2.y)*0.5+y;
    drawSingleAxon(muscleMidX, muscleMidY, axonSource.x+x,axonSource.y+axonSource.m*0.5+y,toImage);
    double averageMass = (ni1.m+ni2.m)*0.5;
    if(toImage == 0){
      fill(axonColor);
      textAlign(CENTER);
...
double toMuscleUsable(double f){
...
void drawArrow(double x) {
  ...
  endShape(CLOSE);
  fill(255);
  text(((double)(round(x*2))/10)+" m", x*scaleToFixBug, -3.91*scaleToFixBug);
}
void drawGraphImage() {
  ...
  if (gen >= 1) {
    stroke(0, 160, 0, 255);
    strokeWeight(3);
    double genWidth = 590.0/gen;
    double lineX = 110+genSelected*genWidth;
    ...
    for (int i = 1; i < 101; i++) {
      int c = s[i]-s[i-1];
      if (c >= 25) {
        double y = ((s[i]+s[i-1])/2)/1000.0*100+573;
...
void drawLines(int x, int y, int graphWidth, int graphHeight) {
  ...
  double genWidth = (double)(graphWidth)/gen;
  double best = extreme(1);
  double worst = extreme(-1);
  double meterHeight = (double)(graphHeight)/(best-worst);
  double zero = (best/(best-worst))*gh;
  double unit = setUnit(best, worst);
  graphImage.stroke(150);
  graphImage.strokeWeight(2);
  graphImage.fill(150);
  graphImage.textFont(font, 18);
  graphImage.textAlign(RIGHT);
  for (double i = ceil((worst-(best-worst)/18.0)/unit)*unit; i < best+(best-worst)/18.0;i+=unit) {
    double lineY = y-i*meterHeight+zero;
...
void drawSegBars(int x, int y, int graphWidth, int graphHeight) {
  ...
  for (int i = 0; i < gen; i+=gensPerBar) {
    ...
    double barX1 = x+i*genWidth;
    double barX2 = x+i2*genWidth;
    int cum = 0;
    for (int j = 0; j < 100; j++) {
...
double extreme(double sign) {
  double record = -sign;
  for (int i = 0; i < gen; i++) {
    double toTest = percentile.get(i+1)[(int)(14-sign*14)];
...
double setUnit(double best, double worst) {
  double unit2 = 3*log(best-worst)/log(10)-2;
  if ((unit2+90)%3 < 1) {
...
String showUnit(double i, double unit) {
  if (unit < 1) {
    return nf(i, 0, 2)+"";
  }
  else {
...
void mousePressed() {
  ...
  double mX = mouseX/windowSizeMultiplier;
  double mY = mouseY/windowSizeMultiplier;
...
void drawCreature(Creature cj, double x, double y, int toImage) {
...
void drawCreaturePieces(ArrayList<Node> n, ArrayList<Muscle> m, double x, double y, int toImage) {
  for (int i = 0; i < m.size(); i++) {
...
void drawHistogram(int x, int y, int hw, int hh) {
  ...
  rect(x, y, hw, hh);
  fill(0, 0, 0);
  double barW = (double)hw/barLen;
  double multiplier = (double)hh/maxH*0.9;
  textAlign(LEFT);
  textFont(font, 16);
  stroke(128);
  strokeWeight(2);
  ...
  for (int i = minBar; i <= maxBar; i += 10) {
    ...
    double theX = x+(i-minBar)*barW;
    text(nf((double)i/histBarsPerMeter, 0, 1), theX, y+hh+14);
    line(theX, y, theX, y+hh);
  }
  noStroke();
  for (int i = 0; i < barLen; i++) {
    double h = min(barCounts.get(genSelected)[i]*multiplier, hh);
...
void setup() {
  ...
  randomSeed(SEED);
  noSmooth();
  int screenW = (int)(windowWidth*windowSizeMultiplier);
  int screenH = (int)(windowHeight*windowSizeMultiplier);
...
void draw() {
  ...
  if (menu == 0) {
    ...
    if (gen == -1) {
      ...
      textAlign(CENTER);
      textAlign(RIGHT);
      text((double)(round(percentile.get(min(genSelected, percentile.size()-1))[14]*1000))/1000+" "+fitnessUnit, 700, 160);
  ...
  if (menu == 8) {
    ...
    pushMatrix();
    scale(10.0/scaleToFixBug);
    double transition = 0.5-0.5*cos(min((double)(timer)/60, PI));
    for (int j = 0; j < 1000; j++) {
      Creature cj = c2.get(j);
      ...
      int x2 = j%40;
      int y2 = floor(j/40)+1;
      double x3 = inter(x1, x2, transition);
      double y3 = inter(y1, y2, transition);
  ...
  if (menu == 10) {
    //Kill!
    for (int j = 0; j < 500; j++) {
      double f = (double)(j)/1000;
      double rand = (pow(random(-1, 1), 3)+1)/2; //cube function
      slowDies = (f <= rand);
      int j2;
      int j3;
      if (slowDies) {
...
void drawStats(double x, double y, double size){
  textAlign(RIGHT);
  textFont(font, 32);
  fill(0);
  pushMatrix();
