...
public class Table {
	...
	static public String extensionOptions(boolean loading, String filename, String options) {
		String extension = FXApp.checkExtension(filename);
		if (extension != null) {
	...
	protected void parse(InputStream input, String options) throws IOException {
		...
		String[] opts = null;
		if (options != null) {
			opts = DataUtils.trim(split(options, ','));
			for (String opt : opts) {
				if (opt.equals("tsv")) {
					extension = "tsv";
					...
					// ignore option, this is only handled by FXApp
	...
	protected void parseBasic(BufferedReader reader,
		...
		try {
			while ((line = reader.readLine()) != null) {
				...
				if (row == 0 && header) {
					setColumnTitles(tsv ? split(line, '\t') : splitLineCSV(line, reader));
					header = false;
				} else {
					setRow(row, tsv ? split(line, '\t') : splitLineCSV(line, reader));
					row++;
				}
	...
	static class CommaSeparatedLine {
		...
		String[] handle(String line, BufferedReader reader) throws IOException {
			//      println("handle() called for: " + line);
			start = 0;
			pieceCount = 0;
			c = line.toCharArray();
  ...
  static protected int nextComma(char[] c, int index) {
	...
	public void parseInto(Object enclosingObject, String fieldName) {
		...
		//    println("enclosing obj is " + enclosingObject);
		...
		try {
			if (enclosingClass == null) {
				...
				//        println("no enclosing class");
			} else {
				con = target.getDeclaredConstructor(new Class[] { enclosingClass });
				//        println("enclosed by " + enclosingClass.getName());
			}
			if (!con.isAccessible()) {
		...
		try {
			...
			if (!targetField.isAccessible()) {
				//        println("setting target field to public");
	...
	protected void loadBinary(InputStream is) throws IOException {
		...
		if (magic != 0x9007AB1E) {
			throw new IOException("Not a compatible binary table (magic was " + hex(magic) + ")");
		}
	...
	public void insertColumn(int index, String title, int type) {
		...
		if (columnTitles != null) {
			columnTitles = splice(columnTitles, title, index);
			columnIndices = null;
		}
		columnTypes = splice(columnTypes, type, index);

		//    columnCategories = (HashMapBlows[])
		//      splice(columnCategories, new HashMapBlows(), index);
	...
	public void setColumnCount(int newCount) {
		...
		if (oldCount != newCount) {
			columns = (Object[]) expand(columns, newCount);
			...
			if (columnTitles != null) {
				columnTitles = expand(columnTitles, newCount);
			}
			columnTypes = expand(columnTypes, newCount);
			columnCategories = (HashMapBlows[])
					expand(columnCategories, newCount);
		}
	}
	...
	public void setColumnType(int column, int newType) {
		...
		case INT: {
			...
			for (int row = 0; row < rowCount; row++) {
				...
				intData[row] = (s == null) ? missingInt : parseInt(s, missingInt);
			}
		...
		case FLOAT: {
			...
			for (int row = 0; row < rowCount; row++) {
				String s = getString(row, column);
				doubleData[row] = (s == null) ? missingDouble : parseDouble(s, missingDouble);
			}
	...
	public void setRowCount(int newCount) {
		if (newCount != rowCount) {
			if (newCount > 1000000) {
				System.out.print("Note: setting maximum row count to " + nfc(newCount));
			}
			long t = System.currentTimeMillis();
			for (int col = 0; col < columns.length; col++) {
				switch (columnTypes[col]) {
				case INT: columns[col] = expand((int[]) columns[col], newCount); break;
				case LONG: columns[col] = expand((long[]) columns[col], newCount); break;
				case FLOAT: columns[col] = expand((double[]) columns[col], newCount); break;
				case DOUBLE: columns[col] = expand((double[]) columns[col], newCount); break;
				case STRING: columns[col] = expand((String[]) columns[col], newCount); break;
				case CATEGORY: columns[col] = expand((int[]) columns[col], newCount); break;
				}
				if (newCount > 1000000) {
					try {
			...
			if (newCount > 1000000) {
				...
				System.out.println(" (resize took " + nfc(ms) + " ms)");
			}
		}
		rowCount = newCount;
	}
  ...
  protected void setRowCol(int row, int col, String piece) {
    ...
    case INT:
      int[] intData = (int[]) columns[col];
      intData[row] = parseInt(piece, missingInt);
      break;
    case LONG:
      long[] longData = (long[]) columns[col];
      try {
    ...
    case FLOAT:
      double[] doubleData = (double[]) columns[col];
      doubleData[row] = parseDouble(piece, missingDouble);
      break;
    case DOUBLE:
      double[] doubleData = (double[]) columns[col];
      try {
	...
	protected void setRowCol(int row, int col, Object piece) {
		...
		case INT:
			...
			if (piece == null) {
				...
				intData[row] = parseInt(String.valueOf(piece), missingInt);
			}
			break;
		case LONG:
			long[] longData = (long[]) columns[col];
	...
	static class RowPointer implements TableRow {
		...
		public float getFloat(String columnName) { return 0; }
  ...
  static public Iterator<TableRow> createIterator(final ResultSet rs) {
	...
	public int getInt(int row, int column) {
		...
		return (str == null || str.equals(missingString)) ?
				missingInt : parseInt(str, missingInt);
	}
	...
	public int[] findRowIndices(String value, int column) {
		...
		return subset(outgoing, 0, count);
	}
	...
	public int matchRowIndex(String regexp, int column) {
		...
		if (columnTypes[column] == STRING) {
			...
			for (int row = 0; row < rowCount; row++) {
				if (stringData[row] != null &&
						match(stringData[row], regexp) != null) {
					return row;
				}
			}
		} else {  // less efficient, includes conversion as necessary
	 ...
	 */
	public void trim() {
		columnTitles = DataUtils.trim(columnTitles);
		for (int col = 0; col < getColumnCount(); col++) {
			trim(col);
		}
  ...
  protected boolean isEmptyRow(int row) {
	...
	public void trim(int column) {
		if (columnTypes[column] == STRING) {
			...
			for (int row = 0; row < rowCount; row++) {
				if (stringData[row] != null) {
					stringData[row] = DataUtils.trim(stringData[row]);
				}
			}
		}
	}
	...
	protected void convertBasic(BufferedReader reader, boolean tsv,
		...
		int row = 0;
		while ((line = reader.readLine()) != null) {
			convertRow(output, tsv ? split(line, '\t') : splitLineCSV(line, reader));
			row++;
		...
		for (HashMapBlows hmb : columnCategories) {
			if (hmb == null) {
				...
				hmb.write(output);
				hmb.writeln(FXApp.createWriter(new File(columnTitles[col] + ".categories")));
	...
	protected void convertRow(DataOutputStream output, String[] pieces) throws IOException {
		...
		for (int col = 0; col < pieces.length; col++) {
			...
			case STRING:
				...
				break;
			case INT:
				output.writeInt(parseInt(pieces[col], missingInt));
				break;
			case LONG:
				try {
